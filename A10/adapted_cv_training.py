...
from models import (
    ...
    split_and_window_sequences, split_flat_sequences,
...
MODEL_CONFIGS = {
    ...
    'dense_windowed': {
        ...
        'data_type': 'windowed', #ValueError: Input 0 with name 'xy_input' of layer 'DenseModel' is incompatible with the layer: expected shape=(None, 26), found shape=(None, 30, 26)
    },
    'conv1d': {
...
def train_model_with_cv(X, y, model_name, model_config, save_dir, verbose=1, test_size=0.1):
    ...
    Args:
        X: Input data (dict with 'flat' and 'windowed' keys, each containing tuple of (train, test))
        y: Target data (dict with 'flat' and 'windowed_last' keys, each containing tuple of (train, test))
        model_name: Name of the model
        model_config: Configuration dictionary for the model
        save_dir: Directory to save results
        verbose: Verbosity level
        test_size: Proportion of data to use for testing (default 0.2) - not used with new split functions
    ...
    if data_type == 'flat':
        X_trainval, X_test = X['flat']
        y_trainval, y_test = y['flat']
    else:  # windowed
        X_trainval, X_test = X['windowed']
        y_trainval, y_test = y['windowed_last']

    print(f"Data shape:")
    print(f"  Training+Validation: X={X_trainval.shape}, y={y_trainval.shape}")
    ...
    # Plot predictions vs actual (use combined train+val data)
    X_combined = np.concatenate([X_trainval, X_test], axis=0)
    y_combined = np.concatenate([y_trainval, y_test], axis=0)
    plot_predictions(best_models, X_combined, y_combined, model_name, save_dir)
...
def main():
    ...
    print(f"\nTotal sessions: {len(sequences)}")

    # Prepare data using split functions to prevent data leakage
    # Split by session first, then window within each split
    ...
    # Windowed data with proper train/test split
    (
    ...
    X = {
        'flat': (X_flat_train, X_flat_test),
        'windowed': (X_seq_train, X_seq_test),
    }
    y = {
        'flat': (y_flat_train, y_flat_test),
        'windowed_last': (y_train_last, y_test_last),
    }

    print(f"\nFlat dataset:")
    print(f"  Train: X={X_flat_train.shape}  y={y_flat_train.shape}")
    print(f"  Test:  X={X_flat_test.shape}  y={y_flat_test.shape}")
    print(f"Windowed dataset:")
    print(f"  Train: X={X_seq_train.shape}  y_last={y_train_last.shape}")
    print(f"  Test:  X={X_seq_test.shape}  y_last={y_test_last.shape}")
